...
# local bucket under exports/<platform>_<id>/ so BOTH mirrors (RECORDING_DIR + S3) carry it.
#
...
for sp in speakers: print("  -", sp, "(", sum(1 for s in segs if (s.get("speaker") or "<unnamed>")==sp), "segments )")'
  ...
  export)
    platform="${3:-google_meet}"
    out="$RECORDING_DIR/exports/${platform}_${nid}"; mkdir -p "$out"
    api GET "/transcripts/$platform/$nid"            > "$out/transcript.json"
...
print(" ".join(str(r["id"]) for r in items if "id" in r))' "$out/recordings.json"); do
    ...
    docker run --rm --network "$NETWORK" -v "$out":/exp:ro --entrypoint sh "$MC_IMAGE" -c \
      "mc alias set store http://$MINIO_CONTAINER:9000 $MINIO_ACCESS_KEY $MINIO_SECRET_KEY >/dev/null && mc cp --recursive /exp/ store/$MINIO_BUCKET/exports/${platform}_${nid}/" >/dev/null \
      && echo "copied JSON into bucket: exports/${platform}_${nid}/ (mirrors → RECORDING_DIR + S3)"
    ;;
  *) usage ;;
esac
