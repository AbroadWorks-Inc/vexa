...
Two Zoom-specific behaviours vs. meet-bot:
  ...
  * The legacy single-shot ``RecordingService.upload()`` (index.ts graceful
    leave) also POSTs the whole WAV here, with NO ``chunk_seq``. Accepting it
    would double the audio, so chunk-less uploads are ignored — the incremental
    ``uploadChunk`` stream (each with a ``chunk_seq``) is the source of truth.
...
async def receive_chunk(
    ...
    # accepting it would concatenate the full recording on top of the per-chunk
    # PCM and double the audio. Ignore chunk-less uploads.
    ...
    try:
        ...
        logger.info(
            ...
            "the source of truth for Zoom audio"
        )
