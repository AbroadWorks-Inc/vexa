...
is never invited, so Google Meet always shows it "Ask to join" and a human has to
admit it. Clicking that button 10 minutes early means knocking on the door of an
empty room: there is nobody present to admit the bot, and the admission budget
(``waitingRoomTimeout``, 15 min) is then spent almost entirely BEFORE the meeting
begins. Observed live 2026-08-19: bot knocked 23:50, owner joined 00:00, bot gave
up at 00:05 with ``admission_timeout`` — only 5 of its 15 minutes of patience
overlapped with a human being in the room.
...
#: Knock this many seconds before the scheduled start. Not zero: Meet's pre-join
#: page needs a moment to load and the click itself takes time, and a bot already
#: in the waiting room when the owner arrives is admitted faster than one that
#: has not asked yet.
#:
